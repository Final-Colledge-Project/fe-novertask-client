...
export default function AddUserToGroupPopup({
  ...
  // check if a user is owner of workspace
  const isOwner = (userId: string) => {
    // admin list always have 1 item
    const wsOwner = allMembers?.workspaceAdmins[0]
    if (wsOwner) {
      return wsOwner.user?._id === userId
    }
    return false
  }
  ...
  const filterDataBySearchString = (data: IMockUser[] | IOtherGroup[]) => {
    let result: IMockUser[] | IOtherGroup[] = []
    ...
    if (searchType === SEARCH_TYPES.USER) {
      ...
      if ('email' in data[0]) {
        result = (data as IMockUser[]).filter((member) => {
          return (
            member.fullName
              .toLowerCase()
  ...
  const unAssignedMembers = useMemo(() => {
    if (allMembers && permission) {
      const reSultBeforePickData = [
        ...cloneDeep(allMembers.workspaceAdmins),
        ...cloneDeep(allMembers.workspaceMembers)
      ]
      const result = reSultBeforePickData.filter((member) => member.user)
      // find the viewer permission => get memberIds
      const viewerPermission = currentWSPermission?.find((item) => {
        return item.isWSViewer
      })
  ...
  const ortherGroupList = useMemo(() => {
    ...
    if (
      !currentWSPermission ||
      !allMembers ||
      !allMembers.workspaceAdmins ||
      !allMembers.workspaceMembers
    )
      return []

    currentWSPermission.forEach((per: IWSPermission) => {
      // do not show viewer permission, because it is already shown in unAssignedMembers
      if (!per.isWSViewer || (per.isWSAdmin && per._id === permission._id)) {
        computedList.push({
          _id: per._id,
          name: per.name,
          memberIds: per.memberIds,
          members: [] as unknown as IMockUser[],
          isWsAdmin: !!per.isWSAdmin
        })
      }
    })
    ...
    const result = computedList.map((group) => {
      group.members = [
        ...allMembers.workspaceAdmins
          .filter((member) =>
            member
              ? group.memberIds.includes(member.user?._id as string)
              : false
          )
          .map((member) => member.user as IMockUser),
        ...allMembers.workspaceMembers
          .filter((member) =>
            member
              ? group.memberIds.includes(member.user?._id as string)
      ...
      group.memberIds = group.members?.map((member) => member._id as string)

      return group
    })

    return filterDataBySearchString(result) as IOtherGroup[]
  }, [currentWSPermission, allMembers, permission._id, searchString])
