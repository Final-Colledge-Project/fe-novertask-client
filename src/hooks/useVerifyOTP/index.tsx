...
const useVerifyOTP = ({ onSubmitCode, length }: IProps) => {
  ...
  return {
    InputCodeElement: (
      <div className="verify-otp-inputs" ref={ref}>
        {inputCodes.map((c, i) => (
          <input
            key={`input-box-${i}`}
            pattern="[0-9]*"
            ...
            placeholder="0"
            className={clsx(
              'verify-otp-inputs__box',
              dirties[i] ? 'verify-otp-inputs__box--dirty' : ''
            )}
            value={inputCodes[i]}
